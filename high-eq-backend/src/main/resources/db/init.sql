...
CREATE TABLE IF NOT EXISTS `history` (
    ...
    `chat_image` LONGTEXT NULL COMMENT '聊天截图(Base64或URL)',
    `role_background` VARCHAR(100) NOT NULL,
    `user_intent` TEXT NOT NULL,
    `model_used` VARCHAR(50) NOT NULL DEFAULT 'deepseek-v4-flash',
...
CREATE TABLE IF NOT EXISTS `reply_suggestion` (
    ...
    `style_label` VARCHAR(32) NULL COMMENT '回复风格标签',
...
CREATE TABLE IF NOT EXISTS `reply_chat_session` (
...
CREATE TABLE IF NOT EXISTS `reply_chat_message` (
    `id` VARCHAR(64) PRIMARY KEY,
...
CREATE TABLE IF NOT EXISTS `reply_chat_suggestion` (
    `id` VARCHAR(64) PRIMARY KEY,
    `session_id` VARCHAR(64) NOT NULL,
